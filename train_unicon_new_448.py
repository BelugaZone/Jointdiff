...
from seghead.segdecoder_448 import seg_decorder
from patch.share_timesteps import current_timestep , set_current_timestep
...
def compute_sqrt_alpha_bar(scheduler, t):
    ...
    sqrt_alpha_bar = sqrt_alpha_bar[half:,:,:,:]

    

    # 计算平方根
    return torch.sqrt(sqrt_alpha_bar)

def create_differential_mask(original_size=(256, 256), target_size=(448, 256), batch_sizes=(4, 4)):
    """
    为混合批次创建差异化注意力掩码
    仅对第二个批次应用填充区域的掩码
    ...
    Args:
        ...
        target_size: 目标填充尺寸 (width, height)
        batch_sizes: 两个批次的样本数量 (batch_16ch, batch_bev)
    
    Returns:
        attention_mask: 差异化掩码张量 [batch_16ch + batch_bev, sequence_length, 1]
        latent_size: 隐空间尺寸 (W, H)
    """
    w_orig, h_orig = original_size
    w_target, h_target = target_size
    batch_16ch, batch_bev = batch_sizes
    
    # 计算VAE下采样后的隐空间尺寸
    downscale_factor = 8
    w_latent = w_target // downscale_factor  # 448/8 = 56
    h_latent = h_target // downscale_factor  # 256/8 = 32
    sequence_length = w_latent * h_latent  # 56 * 32=1792
    
    # 计算填充区域在隐空间的宽度
    pad_total = w_target - w_orig
    pad_latent = (pad_total // 2) // downscale_factor  # (192/2)/8 = 12
    
    # ===== 创建两种不同的掩码 =====
    
    # 1. 无掩码版本 (第一个批次) - 全1张量
    mask_full = torch.ones(1, sequence_length)  # [1, 1792]
    
    # 2. 带填充掩码版本 (第二个批次)
    latent_mask = torch.ones((h_latent, w_latent))  # [32, 56]
    
    # 应用填充区域掩码
    # 左侧填充区域: 前12列
    latent_mask[:, :pad_latent] = 0.0
    # 右侧填充区域: 最后12列
    latent_mask[:, -pad_latent:] = 0.0
    
    # 展平为二维 [1, key_tokens]
    mask_padded = latent_mask.flatten().unsqueeze(0)  # [1, 1792]
    mask_padded = mask_padded.repeat(batch_bev, 1)    # [batch_bev, 1792]
    
    # ===== 修正维度匹配 =====
    # 确保两部分都是3维张量 (batch_size, 1, seq_len)
    part1 = mask_full.repeat(batch_16ch, 1).unsqueeze(1)  # [batch_16ch, 1, 1792]
    part2 = mask_padded.unsqueeze(1)                     # [batch_bev, 1, 1792]
    
    # 在批次维度拼接
    attention_mask = torch.cat([part1, part2], dim=0)    # [batch_16ch+batch_bev, 1, 1792]
    
    latent_size = (w_latent, h_latent)
    return attention_mask, latent_size
...
def main():
    ...
    seg_head = seg_decorder((256, 448)).cuda()
    ...
    #          "diffusion_pytorch_model.safetensors" if args.variant == "fp16" else "pytorch_model.bin"
    #     )
    ...
    #             for key in f.keys():
    #                 if key not in ignore_keys:  # 过滤键
    #                    state_dict[key] = f.get_tensor(key)
    #     else:
    #         state_dict = torch.load(model_path, map_location="cpu")
    #         for key in ignore_keys:
    #             state_dict.pop(key, None)  # 安全删除键

    # # 步骤4: 加载修改后的状态字典
    #     missing_keys, unexpected_keys = unet.load_state_dict(state_dict, strict=False)
    #     print(f"忽略的键: {ignore_keys}")
    #     print(f"缺失的键: {missing_keys}, 意外的键: {unexpected_keys}")
    ...
    for name, param in unet.named_parameters():
        if param.is_floating_point():
            ...
            print(f"警告: 参数 {name} 不是浮点类型 ({param.dtype})，无法计算梯度")
            param.requires_grad = False  # 确保设为False
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # print(batch["intrinsics_origin"][0])
                ...
                # img = batch["image"][0]
                # save_dir_img = "/opt/data/private/hwj_autodrive/UniCon/groundtruth/gt_img"
                # seg , project = process_segmentation(c[0])
                # image = process_rgb(img)
                # save_dir = "/opt/data/private/hwj_autodrive/UniCon/groundtruth/gt_bev"
                ...
                batch_half = latents_16ch.shape[0]

                # 应用尺寸掩码
                attention_mask, latent_size = create_differential_mask(
                        (256,256), 
                        (448,256), 
                ...
                set_current_timestep(timesteps)
                ...
                # 分割头分割
                seg_outputs = seg_head(diffusion_features)
                # print('seg_outputs',seg_outputs.shape)

                seg_target = c
                seg_target = (seg_target + 1) / 2

                sqrt_alpha_bar_t = compute_sqrt_alpha_bar(noise_scheduler, timesteps)
                ...
                if args.snr_gamma is None:
                    ...
                    # 获取batch大小和分割点
                    half = bsz // 2
                    spatial_mask = attention_mask.view(bsz, 32, 56, 1)
                    spatial_mask = spatial_mask.permute(0, 3, 1, 2)  # [B, 1, H, W]
                    ...
                    bev_mask = spatial_mask[half:]
                    bev_mask = bev_mask.to(bev_pred.device)
                    ...
                    loss_bev_per_sample = loss_bev_per_sample * bev_mask.expand(-1, bev_pred.shape[1], -1, -1)
                    ...
                    # )
                    original_h, original_w = 256, 448
                    valid_h, valid_w = 256, 256  # 有效区域尺寸

                    # 计算宽度方向的填充量
                    padding_left = (original_w - valid_w) // 2  # (448-256)/2 = 96
                    padding_right = original_w - valid_w - padding_left  # 448-256-96=96

                    # 定义有效区域边界
                    h_start, h_end = 0, original_h  # 高度方向全部有效 (0-256)
                    w_start, w_end = padding_left, padding_left + valid_w  # 96, 352 (96+256=352)

                    # 裁剪有效区域
                    cropped_outputs = seg_outputs[:, :, h_start:h_end, w_start:w_end]
                    cropped_target = seg_target[:, :, h_start:h_end, w_start:w_end]
                    ...
                    # 总损失为平均值
                    loss = (loss_rgb + loss_bev + loss_bce) / 3.0
                    # loss = loss_bev
                # 获取损失值用于后续记录
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log({
                    ...
                    "train_loss_perceptual": avg_loss_perceptual  # BEV数据损失
                }, step=global_step)
