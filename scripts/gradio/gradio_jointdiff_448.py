...
def process(image_input, cond_input, prompt, model_selection, additional_prompt, negative_prompt, y_prompt, enable_joint_attn, batch_size, height, width, num_inference_step, seed, scheduler_selection, base_model_selection, guidance_scale, joint_scale, image_noise_strength, cond_noise_strength, cond_guidance_scale):
    ...
    if base_model_id != pipe.unet.base_model_id:
        ...
        if cur_model not in pipe.unet.jointdiff_adapters:
            ...
            active_adapters = load_jointdiff_weights(pipe.unet, checkpoint_path, post_joint, model_name = model_name, adapter_names = adapter_names)
        else:
            active_adapters = pipe.unet.jointdiff_adapters[cur_model]
    ...
    with torch.no_grad():
        ...
        # x_images = images[:batch_size]
        x_images = x_images[:, :3, :, :]
        # y_images = images[batch_size:]

        

        # 对每个 y 图像应用 viz_bev 渲染逻辑
        def process_segmentation(y_img_pil):
            ...
            y_tensor = resize(y_tensor, (256, 256), order=1, anti_aliasing=True)
            ...
            return colored_seg.pil , bev_cam_imge.pil, bev_cam_imge.pil
        ...
        processed_y_images = [img for y_img in y_images for img in process_segmentation(y_img)]
        processed_x_images = [process_rgb(x_img) for x_img in x_images]

        # 合并处理后的图像用于显示
        processed_images = processed_x_images + processed_y_images
        save_dir = "/opt/data/private/hwj_autodrive/jointdiff/output/gallery"
        os.makedirs(save_dir, exist_ok=True)
       ...
       # 遍历所有图片并保存
        ...
        processed_image_grid = make_image_grid([*init_images, *processed_images], rows=2, cols=batch_size * 3)

         # 返回处理后的图像（仅影响 Gradio 显示）
        return [processed_image_grid, *processed_images]
...
with demo:
    with gr.Row():
        gr.Markdown("## jointdiff models demo")
    gr.Markdown("""
1. Upload an image and/or a condition input
2. Provide a prompt (or click "Generate Caption") and select the jointdiff model corresponding to your image adnd condition
...
There are examples of different jointdiff models and inference tasks below for you to get familiar with all the options.
""")
    with gr.Row():
        with gr.Column():
            with gr.Row():
            ...
            cond_noise_strength = gr.Slider(label="condition noise strength", minimum=0.0, maximum=1.0, value=1.0, step=0.05)
            with gr.Accordion("Advanced options", open=False):
                ...
                height = gr.Slider(label="height", minimum=128, maximum=1024, value=256, step=64)
                width = gr.Slider(label="width ", minimum=128, maximum=1024, value=448, step=64)
                ...
                guidance_scale = gr.Number(label="CFG guidance sacle", value=1.5)
                joint_scale = gr.Slider(label="joint attention scale", minimum=0, maximum = 2, value = 1, step = 0.1)
                cond_guidance_scale = gr.Number(label="condition guidance scale", value=1.0)
