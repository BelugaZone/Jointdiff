...
def combine_images(img_folder, gt_folder, pred_folder, output_folder, font_path=None):
    ...
    for filename in os.listdir(img_folder):
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            # 提取基本名（如"img0" -> "0"）
            base_name = filename.split('gt_img_')[-1].split('.')[0]
    ...
    for base_name in sorted_base_names:
        # 构建三个图像文件的完整路径
        target_size = (448, 256) 
        img_path = os.path.join(img_folder, f"gt_img_{base_name}.jpg")
        gt_path = os.path.join(gt_folder, f"bev4c_label_{base_name}.png")
        pred_path = os.path.join(pred_folder, f"imggen_{base_name}.jpg")
...
if __name__ == "__main__":
    combine_images(
        img_folder='/opt/data/private/hwj_autodrive/jointdiff/AAstanderd_imggen/448/imggen_finalmodel_448_45000:21.83/img_gt',
        gt_folder='/opt/data/private/hwj_autodrive/jointdiff/AAstanderd_imggen/448/imggen_finalmodel_448_45000:21.83/bev_4c',
        pred_folder='/opt/data/private/hwj_autodrive/jointdiff/AAstanderd_imggen/448/imggen_finalmodel_448_45000:21.83/img_gen',
        output_folder='/opt/data/private/hwj_autodrive/jointdiff/AAstanderd_imggen/448/imggen_finalmodel_448_45000:21.83/combined',
    )