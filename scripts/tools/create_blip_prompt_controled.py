...
def generate_and_save_captions(dataset, output_path, batch_size=16):
    ...
    # 使用DataLoader迭代处理
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Generating Scene Captions")):
        # 准备图像
        pil_images = []
        ...
        try:
            ...
            for img in pil_images:
                try:
                    ...
                    # 回退默认描述
                    captions.append("a realistic driving scene")
        
        # 收集结果
        for i in range(len(pil_images)):
            # 安全获取元数据，防止索引越界
            token_val = batch.get("token")
            if token_val is not None and isinstance(token_val, list) and i < len(token_val):
                token = token_val[i]
            else:
                token = str(sample_count)
                
            scene_val = batch.get("scene_name")
            if scene_val is not None and isinstance(scene_val, list) and i < len(scene_val):
                scene_name = scene_val[i]
            else:
                scene_name = "unknown"
                
            time_val = batch.get("timestamp")
            if time_val is not None and isinstance(time_val, list) and i < len(time_val):
                timestamp = time_val[i]
            else:
                timestamp = 0
...
if __name__ == "__main__":
    ...
    print("Starting scene caption generation for NuScenes dataset...")
    captions = generate_and_save_captions(
        dataset=train_dataset,
        output_path="nuscenes_scene_captions.json",
    ...
    print(f"\nCompleted! Generated {len(captions)} scene captions saved to nuscenes_scene_captions.json")