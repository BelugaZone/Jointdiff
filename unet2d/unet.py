...
class UNet2D(UNet2DConditionModel):
    ...
    def forward(
        ...
        if flag_time and all_feature_dic:
            batch_size = sample.size(0)
            start = batch_size // 2
            end = batch_size

            reshape_h = sample[start:end,:,:,:]

            # reshape_h=bev_sample.reshape(int(bev_sample.size()[0]/2),int(bev_sample.size()[1]*2),bev_sample.size()[2],bev_sample.size()[3])
            
            if reshape_h.size()[2]==4:
                all_feature_dic["low"].append(reshape_h)
            elif reshape_h.size()[2]==8:
        ...
        for downsample_block in self.down_blocks:
            ...
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                class_name = type(downsample_block).__name__
                # print(f"处理 {class_name} 类型的下采样块 (位置: down_blocks")
                # print('非cross')
...
#                     print(reshape_h.size()[2])
        ...
        if self.mid_block is not None:
            ...
            if hasattr(self.mid_block, "has_cross_attention") and self.mid_block.has_cross_attention:
                ...
                # forward_signature = inspect.signature(self.mid_block.forward)
                # accepts_timestep = "timestep" in forward_signature.parameters
                ...
                # sample = self.mid_block(**call_kwargs)
            else:
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if hasattr(upsample_block, "has_cross_attention") and upsample_block.has_cross_attention:
                ...
                # forward_signature = inspect.signature(upsample_block.forward)
                # accepts_timestep = "timestep" in forward_signature.parameters
                ...
                # sample = upsample_block(**call_kwargs)
            ...
            if flag_time and all_feature_dic:
                ...
                if reshape_h.size()[2]==4:
                    ...
                    #     feature_map = feature_map[1]  # 取第一个批次

                    # # 提取指定通道
                    # channel_data = feature_map[1]
