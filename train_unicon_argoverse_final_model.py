...
import itertools
from argoverse_dataset.dataset import argoverseDatasetConfig as DatasetConfig_argoverse
from argoverse_dataset.data_factory import build_dataloader as build_dataloader_argoverse
from argoverse_dataset.data_factory import build_argoverse_datasets
...
def main():
    ...
    def scale_intrinsics(intrinsics, orig_size, target_size):
        """
        支持批处理的相机内参缩放
        ...
        # 确保是三维张量 (批次, 行, 列)
        if intrinsics.dim() == 2:
            intrinsics = intrinsics.unsqueeze(0)  # 添加批次维度
        ...
        # 创建缩放矩阵 (B, 2, 2)
        batch_size = intrinsics.size(0)
        scale_matrix = torch.eye(2, device=intrinsics.device, dtype=intrinsics.dtype)
        scale_matrix = scale_matrix.unsqueeze(0).repeat(batch_size, 1, 1)  # (B, 2, 2)
        scale_matrix[:, 0, 0] = scale_x
        scale_matrix[:, 1, 1] = scale_y
        
        # 提取内参矩阵的前两行 (B, 2, 3)
        intrinsics_top = intrinsics[:, :2, :]
        
        # 执行批处理矩阵乘法
        scaled_top = torch.bmm(scale_matrix, intrinsics_top)
        
        # 创建新的内参矩阵
        scaled_intrinsics = intrinsics.clone()
        scaled_intrinsics[:, :2, :] = scaled_top
        
        # 如果输入是二维，移除批次维度
        if intrinsics.dim() == 2:
            scaled_intrinsics = scaled_intrinsics.squeeze(0)
            
        return scaled_intrinsics

    with open("/opt/data/private/hwj_autodrive/UniCon/config/data_confg/argoverse.yaml", "r") as f:
        config_dict = yaml.safe_load(f)
    data_cfg = DatasetConfig_argoverse(cfg=config_dict)

    argoverse_dataset , val_data = build_argoverse_datasets(data_cfg)
    dataset_length = len(argoverse_dataset)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # 缩放内参矩阵
                camera_intrinsic = scale_intrinsics(camera_intrinsic, orig_size, target_size)
                ...
                # timesteps_front = torch.ones(half, dtype=torch.long)
                ...
                c = c[:,:6,:,:]
                # c = c[:,[0,1,2,8,10,11],:,:]
                # first_channel = c[:, 0:1, :, :]  # 保持维度 [batch_size, 1, height, width]

                # # 复制第一个通道到所有6个通道
                # c = first_channel.expand(-1, 6, -1, -1) 
                ...
                model_pred = unet(masked_input, timestep=timesteps, encoder_hidden_states=encoder_hidden_states,camera_intrinsic=camera_intrinsic, return_dict=False)[0]
