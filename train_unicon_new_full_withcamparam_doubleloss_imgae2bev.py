...
def main():
    ...
    if args.resume_from_model is not None:
        ...
        if args.resume_from_model.endswith(".pth"):
            ...
            state_dict = {k.replace("conv1n", f"post1n.{'depth'}"): v for k, v in state_dict.items()}
    ...
    def freeze_specific_parameters(model):
        """冻结指定的参数"""
        params_to_freeze = [
            "conv_out.bias", "conv_out.weight",
            "conv_norm_out.bias", "conv_norm_out.weight"
        ]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # if mode == 0:  # 原始策略：整个批次随机
                #     timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,))
                
                # elif mode == 1:  # 策略1：相同时间步（仅前半采样）
                #     timesteps_half = torch.randint(0, noise_scheduler.config.num_train_timesteps, (half,))
                #     timesteps = torch.cat([timesteps_half, timesteps_half.clone()])
                ...
                #     timesteps_front = torch.randint(0, noise_scheduler.config.num_train_timesteps, (half,))
                #     timesteps_back = torch.zeros(half, dtype=torch.long)
                #     timesteps = torch.cat([timesteps_front, timesteps_back])
                ...
                # timesteps_back = torch.ones(half, dtype=torch.long)
                timesteps_back = torch.randint(0, noise_scheduler.config.num_train_timesteps, (half,))
                timesteps = torch.cat([timesteps_front, timesteps_back])
                ...
                # #    模式1: 联合生成 (原始模式)
                #     rgb_noise = torch.randn_like(latents_16ch)
                #     bev_noise = torch.randn_like(latents_bev)
                #     noise = torch.cat([rgb_noise, bev_noise], dim=0)
                # else:
                # # 模式2: 条件生成 (新功能)
                #     rgb_noise = torch.zeros_like(latents_16ch)
                #     bev_noise = torch.randn_like(latents_bev)
                #     noise = torch.cat([rgb_noise.detach(), bev_noise], dim=0)
                ...
                if args.snr_gamma is None:
                    ...
                    # 总损失为平均值
                    # loss = (loss_rgb + loss_bev + loss_bce + loss_bce_img2bev) / 4.0
                    loss = (loss_bce_img2bev + loss_bce)  / 2.0
                # 获取损失值用于后续记录
