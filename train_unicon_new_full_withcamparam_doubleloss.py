...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_dir",
        type=str,
        default="/opt/data/private/nuscenes",  # 默认值
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # first_channel = c[:, 0:1, :, :]  # 保持维度不变 [batch, 1, H, W]
                # # 将后两个通道（索引4和5）替换为第一个通道的复制
                # c[:, 4:5, :, :] = first_channel  # 第5个通道（索引4）
                # c[:, 5:6, :, :] = first_channel  # 第6个通道（索引5）
