...
STATIC = ['drivable_area', 'ped_crossing', 'walkway', 'carpark_area']
...
Three_class = ["road_segment","car","truck"]
...
def encode(x):
    ...
    assert all(x in [0, 1] for x in np.unique(x))
...
def get_line_layers(nusc_map, sample, layers, patch_radius=40, thickness=1):
    h, w = bev_shape[:2]
    V = view
    M_inv = np.array(sample["pose_inverse"])
    S = np.array(
...
def get_static_layers(nusc_map, sample, layers, patch_radius=40):
    h, w = bev_shape[:2]
    V = view
    M_inv = np.array(sample["pose_inverse"])
    S = np.array(
