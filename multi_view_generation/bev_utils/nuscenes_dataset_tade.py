...
class NuScenesDataset(Dataset):
    ...
    def get_fov_bev(self, cam_record_data):
        ...
        label_path = os.path.join(self.dataset_dir, "map-labels-v1.2", cam_record_data['cam_token'] + ".png")
        encoded_labels = to_tensor(Image.open(label_path)).long()
        camera_intrinsic = cam_record_data["intrinsic"] 
        ...
        bev_resized = cv2.resize(bev, (200, 200), interpolation=cv2.INTER_LINEAR)

    
        bev_resized = (bev_resized > 0.5)
        bev_resized = bev_resized.transpose(2, 0, 1)

        # Converter = MapConverter(h_polar=512,
        # w_polar=512,
        # h_cart=512,
        # w_cart=512,
        # extents_polar=[0, 53],
        # extents_cart=[-25, 0, 25, 50],
        # cam2img = [[405.25350498, 0, 261.20544632]],
        # image_width=512,)
        Converter = MapConverter( h_polar=256,
        w_polar=704,
        h_cart=200,
        w_cart=200,
        extents_polar=[0, 53],
        ...
        bev_resized =  Converter.cart2polar(bev_resized)
        print('bev_resized',bev_resized.shape)
        bev_resized = bev_resized.permute(1, 2, 0)
        bev_resized = (bev_resized > 0.5)

        bev_resized = bev_resized.to(torch.float32) 
        # bev_resized = bev_resized.type(np.float32)
        bev_resized = 2 * bev_resized - 1 
