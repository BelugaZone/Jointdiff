...
import os
import cv2
from .deformable_detr.position_encoding import LearnedPositionalEncoding
...
class SpatialCrossAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.deformable_attention = DeformableAttention3D(embed_dims=320)
        ...
        self.position_embedding = LearnedPositionalEncoding(160, feat_size_h, feat_size_w)
    ...
    def forward(self,
        ...
        query_pos_reshaped = query_pos_reshaped.reshape(bs, self.feat_size_h*self.feat_size_w, 320)
        ...
        for j in range(bs):
            # 直接将结果写回对应位置
            valid_len = min(max_len, len(index_query_per_img))
            slots[j, index_query_per_img[:valid_len]] = attn_output[j, :valid_len].float()
        ...
        out_q = self.dropout(slots) + inp_residual
