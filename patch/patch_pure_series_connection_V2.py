    ...
    class UniConBlock(block_class):
        ...
        def forward(
            ...
            if self.attn2 is not None:
                ...
                input_x = hidden_states[x_ids]
                input_y = hidden_states[y_ids]
                cond_x = encoder_hidden_states[x_ids]
                cond_y = encoder_hidden_states[y_ids]

                attn_output_y = self.attn2(
                    input_y,
                ...
                attn_output_y2x = self.attn1n_y2x(
                    input_x,
                    encoder_hidden_states=attn_output_y,  # 使用y作为上下文
                    attention_mask=attention_mask,
                    ** cross_attention_kwargs,
                )

                # attn_output = torch.cat([attn_output_y2x, attn_output_y], dim=0)
                ...
                # input_x_2 = hidden_states[x_ids]
                # input_y_2 = hidden_states[y_ids]

                attn_output_x = self.attn2(
                    attn_output_y2x,
                    encoder_hidden_states=cond_x,
                    attention_mask=encoder_attention_mask,
                    **cross_attention_kwargs,
                ...
                attn_output_x2y = self.attn1n_x2y(
                    attn_output_y2x,
                    encoder_hidden_states=attn_output_x,  # 使用x作为上下文
                    attention_mask=attention_mask,
                    ** cross_attention_kwargs,
                )

                # attn_output = torch.cat([attn_output_x, attn_output_x2y], dim=0)
