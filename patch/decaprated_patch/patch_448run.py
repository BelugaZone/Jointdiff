...
def make_diffusers_jointdiff_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class JointdiffBlock(block_class):
        ...
        def initialize_joint_layers(self, post="conv", add_bias=False):
            ...
            self.crossview_y2x = SpatialCrossAttention(
                feat_size_h=32,
                feat_size_w=32,
                img_size=(256,448)
        ...
        def forward(
            ...
            timestep_cur = get_current_timestep()
            ...
            if self.enable_joint_attention:
                ...
                if self.enable_joint_attention and hasattr(self, '_is_outermost') and self._is_outermost:
                   ...
                   # 将输入转换为4D格式：(b, c, H, W)
                    ...
                    valid_H, valid_W = 32, 32  # 高度全部有效，宽度取中间32

                    # 计算宽度方向的填充量
                    padding_left = (W - valid_W) // 2  # (56-32)/2 = 12
                    padding_right = W - valid_W - padding_left  # 56-32-12=12

                    # 将展平的序列恢复为空间网格
                    # 假设input_y形状为 (batch_size, H*W) = (B, 32 * 56)
                    spatial_y = input_y.reshape(b, H, W,c)  # 形状变为 (B, 32, 56)

                    # 提取有效区域
                    valid_spatial_y = spatial_y[:, :, padding_left:padding_left+valid_W,:]  # (B, 32, 32)

                    # 如果需要展平回去
                    valid_y = valid_spatial_y.reshape(b, 32 * 32,c)





                    attn_y2x_valid = self.crossview_y2x(
                         valid_y,
                         input_x,
                         input_x,
                         32,
                    ...
                    restored_spatial = torch.zeros(valid_y.size(0), H, W, c,
                              device=valid_y.device, 
                              dtype=valid_y.dtype)  # 形状 (B, 32, 56)

                    # 将有效区域放入中间位置
                    restored_spatial[:, :, padding_left:padding_left+valid_W,:] = valid_spatial_y

                    # 展平回原始序列长度
                    restored_y = restored_spatial.reshape(b, H * W,c)
                    
                    attn_y2x = restored_y

                    # print("attn_y2x.shape", attn_y2x.shape)
