...
from .cross_view_encoder_V2 import CrossViewAttention, BEVEmbedding
...
def make_diffusers_unicon_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class UniConBlock(block_class):
        ...
        def initialize_joint_layers(self, post="conv", add_bias=False):
            ...
            self.crossview_align = CrossViewAttention(  
                 feat_height = 32,
                 feat_width = 32,
                 feat_dim = 320,
                 dim = 320,
            ...
            if self.enable_joint_attention:
                ...
                if self.enable_joint_attention and hasattr(self, '_is_outermost') and self._is_outermost:
                   ...
                   # 将输入转换为4D格式：(b, c, H, W)
                    ...
                    input_y_4d_flip = torch.flip(input_y_4d, dims = [2])

                    img_feature , bev_feature , img_embed , bev_embed = self.crossview_align(
                         input_y_4d_flip,
                         self.bev_embedding,
                         input_x_4d,
                         intrinsics01,
                    ...
                    img_feature_4D = img_feature + img_embed 
                    bev_feature_4D = bev_feature + bev_embed


                    input_x_cvt = img_feature_4D.permute(0, 2, 3, 1).view(b, n, c)
                    input_y_cvt = bev_feature_4D.permute(0, 2, 3, 1).view(b, n, c)


                    attn_x2y = self.attn1n_x2y(
                    input_x_cvt,
                    encoder_hidden_states=input_y_cvt,  # 使用y作为上下文
                    attention_mask=attention_mask,
                    ** cross_attention_kwargs,)
