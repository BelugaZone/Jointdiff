...
from .LSSformer import LiftSplatShoot
...
def make_diffusers_unicon_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class UniConBlock(block_class):
        ...
        def initialize_joint_layers(self, post="conv", add_bias=False):
            ...
            self.lssformer = LiftSplatShoot(grid_conf, data_aug_conf)
        ...
        def forward(
            ...
            if self.enable_joint_attention:
                ...
                if self.enable_joint_attention and hasattr(self, '_is_outermost') and self._is_outermost:
                    ...
                    # 添加相机维度 (N=1)
                    rots = rots.unsqueeze(1)       # (B, 1, 3, 3)
                    trans = trans.unsqueeze(1)     # (B, 1, 3)
                    intrins = intrinsics01.unsqueeze(1)
    
                   # 将输入转换为4D格式：(b, c, H, W)
                    # input_y_4d = input_y.view(b, H, W, c).permute(0, 3, 1, 2)  # (b, c, H, W)
                    input_x_4d = input_x.view(b, H, W, c).permute(0, 3, 1, 2)  # (b, c, H, W)
                    input_x_5d = input_x_4d.unsqueeze(1)

                    input_x_lss_4d = self.lssformer(input_x_5d, rots, trans, intrins) #（b, cxd, H, W）
                    # print('input_x_lss_4d',input_x_lss_4d.shape)
                    b, c_lss , h_lss , w_lss = input_x_lss_4d.shape

                    input_x_lss = input_x_lss_4d.permute(0, 2, 3, 1).view(b, h_lss*w_lss, c)
