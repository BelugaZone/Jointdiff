...
from .LSSformer_mono import TransformerLiftSplatShoot
# from .share_timesteps import current_timestep, get_current_timestep  
# from .share_timesteps import get_current_camera_intrinsic, camera_intrinsic,set_current_E, get_current_E, camera_E
...
def make_diffusers_jointdiff_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class JointdiffBlock(block_class):
        ...
        def initialize_joint_layers(self, post="conv", add_bias=False):
            ...
            self.lssformer = TransformerLiftSplatShoot()
        ...
        def forward(
                ...
                camera_E: Optional[torch.Tensor] = None,
            ...
            # print('self.camera_E ',self.camera_E )
            # print('self.camera_intrinsic ',camera_intrinsic)
            # self.camera_E 
            ...
            if self.enable_joint_attention:
                ...
                # print(f"x_weights shape: {x_weights.shape}")
                ...
                #     # print('timestep_inpatch',timestep_cur.shape)

                #     bsz = timestep_cur.shape[0]
                #     half = bsz // 2
                #     tx = timestep_cur[:half]
                #     ty = timestep_cur[half:]
                #     total_steps = tx + ty 
            
                #     # 2. 实现公式(6)
                #     # 更纯净分支(ty小)时→ tx/(tx+ty)增大 → wx增加
                #     ratio_x = tx / total_steps  # RGB分支的噪声比例
                #     ratio_y = ty / total_steps  # 深度分支的噪声比例
                    
                #     # 核心逻辑：噪声更大的分支获得更高权重
                #     wx = torch.sigmoid(3 * (ratio_y - 0.5))  # 深度噪声大时→ RGB权重增加
                #     wy = torch.sigmoid(3 * (ratio_x - 0.5))
                #     x_weights = wy
                #     y_weights = wx
                ...
                #     attn_x2y_4d = self.crossview_x2y(
                #          input_y_4d,
                #          self.bev_embedding,
                #          input_x_4d,
                #          intrinsics01,
                ...
                #     attn_x2y = attn_x2y_4d.permute(0, 2, 3, 1).view(b, n, c)
                ...
                if self.enable_joint_attention and hasattr(self, '_is_outermost') and self._is_outermost:
                    ...
                    input_x_lss_4d = self.lssformer(input_x_4d,camera_intrinsic) #（b, cxd, H, W）
                    # print('input_x_lss_4d',input_x_lss_4d.shape)
                    b, c_lss , h_lss , w_lss = input_x_lss_4d.shape

                    input_x_lss = input_x_lss_4d.permute(0, 2, 3, 1).view(b, h_lss*w_lss, c_lss)

                    # print('input_x_lss',input_x_lss.shape)


                    attn_y2x = self.attn1n_y2x(
                    input_y,
                    encoder_hidden_states=input_x_lss,  # 使用x作为上下文
                    attention_mask=attention_mask,
                    ** cross_attention_kwargs,
                    )
...
def apply_patch(
    ...
    for name, module in diffusion_model.named_modules():
        ...
        if isinstance_str(module, "BasicTransformerBlock"):
            ...
            module.is_jointdiff_block = True

            new_class = make_jointdiff_block_fn(module.__class__)
            # new_module = new_class()  # 创建新类的临时实例

            # module.forward = MethodType(new_module.forward, module)
